...
const loginDisplay = document.getElementById('login-container');
const video = document.getElementById('video');
const message = document.getElementById('result');
const train = document.getElementById('btn');
    ...
    setTimeout(() => {
      ...
      try {
        ...
        if (name == 'unknown') {
          ...
          message.innerHTML = ` <p> Sorry You are not a registered user </p> <h1>Login Failed </h1>`
        }
        else {
        ...
        s.style.color = 'red';
        s.innerText = "can't recognize try one more time";
        loginDisplay.append(s);
      }
...
async function loadLabeledImages() {
  const labels = ['Eminem', "Jennifer Lawrence", 'Keanu Reeves'];
  return Promise.all(
